...
function isCreditCardNumber(ccn) {
  const arrReverse = ccn.toString().split('').reverse();
  const arrOdd = arrReverse.map((x, i) => (i % 2 !== 0 ? x * 2 : x));
  const arrNine = arrOdd.map((x) => (x > 9 ? x - 9 : x));
  const arrReduced = arrNine.reduce((a, b) => Number(a) + Number(b), 0);

  return arrReduced % 10 === 0;
}
