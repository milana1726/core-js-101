...
function timeSpanToString(startDate, endDate) {
  ...
  const HH = Math.trunc((diffDate / 3600000) % 100).toString().padStart(2, '0');
  const mm = Math.trunc((diffDate / 60000) % 60).toString().padStart(2, '0');
  const ss = Math.trunc((diffDate / 1000) % 60).toString().padStart(2, '0');
  const sss = Math.trunc(diffDate % 1000).toString().padStart(3, '0');
  return (`${HH}:${mm}:${ss}.${sss}`);
}
